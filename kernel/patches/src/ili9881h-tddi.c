...
static void ili_report(struct ili9881h_ts *ts, const u8 *buf, unsigned int len)
{
	...
	for (i = 0; i < ILI_MAX_FINGERS; i++) {
		...
		x = x * ts->size_x / ILI_TPD_RESOLUTION;
		y = y * ts->size_y / ILI_TPD_RESOLUTION;
...
static irqreturn_t ili9881h_irq(int irq, void *dev_id)
{
	...
	if (ret || !size || size > ILI_MAX_PACKET) {
		ili_ice_disable(ts);
		if (!ret)
			dev_dbg(dev, "implausible packet length %u\n", size);
		return IRQ_HANDLED;
	}
	...
	if (ts->buf[0] == ILI_PKT_DEMO && size >= ILI_DEMO_PACKET_LEN)
		...
		dev_dbg(dev, "ignoring packet id 0x%02x (%u bytes)\n", ts->buf[0], size);
...
static int ili9881h_probe(struct spi_device *spi)
{
	...
	/*
	 * Do not fail the probe when the IC stays quiet: on this hardware the
	 * touch block is powered by the panel, so it only answers once the
	 * display has been brought up.
	 */
	ret = ili_ice_enable(ts);
	if (ret) {
		dev_info(dev, "controller not answering yet (%d), continuing\n", ret);
	} else {
		...
		if (ret)
			dev_info(dev, "could not select demo mode (%d)\n", ret);
	}
